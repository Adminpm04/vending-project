...
CMD_MACHINE_STATUS_REQ = 0x51  # мы → VMC: запросить статус машины (купюро-/монетоприёмник и т.п.)
CMD_MACHINE_STATUS = 0x52      # VMC → мы: статус машины
CMD_ELEVATOR_STATUS_REQ = 0x53  # мы → VMC: запросить статус лифта/дверцы выдачи
CMD_ELEVATOR_STATUS = 0x54      # VMC → мы: статус лифта/дверцы выдачи
CMD_MENU_REQ = 0x70   # мы → VMC: «меню»-команды (то же самое, что сервисный экран/клавиатура на самой плате)
CMD_MENU_RESP = 0x71  # VMC → мы: ответ на меню-команду
...
def build_query_selection_number(pack_no: int) -> bytes:
    """0x70 с command type 0x41 — «какие номера слотов реально знает VMC».
    Формат ответа в документации описан нечётко (раздел 4.5.41) — разбираем
...
def parse_menu_response(text: bytes) -> dict:
    """Разобрать Text пакета 0x71: CommandType(1) + OperationType(1) + остальное.
    "Остальное" — формат зависит от command type и не всегда чётко
    задокументирован, отдаём как raw hex."""
    command_type = text[0] if len(text) >= 1 else None
    operation_type = text[1] if len(text) >= 2 else None
    return {
        "command_type": command_type,
        "operation_type": operation_type,
        "raw_hex": text[2:].hex() if len(text) > 2 else "",
    }


def build_elevator_status_req(pack_no: int) -> bytes:
    """0x53 — запросить статус лифта/дверцы выдачи (общий для всех слотов,
    не завязан на конкретную покупку) — диагностика застрявшего товара."""
    return build_packet(CMD_ELEVATOR_STATUS_REQ, pack_no)


# Статусы лифта/дверцы выдачи (Text[0] пакета 0x54)
ELEVATOR_STATES = {
    0x00: "Normal",
    0x01: "Product stuck in elevator",
...
def parse_elevator_status(text: bytes) -> dict:
    """Разобрать Text пакета 0x54: Status(1)."""
    code = text[0]
    return {"ok": code == 0x00, "code": code, "message": ELEVATOR_STATES.get(code, f"status 0x{code:02X}")}
